...
LayerComp = [[0.505, 0.019, 0.010, 0.467, 0.000], [0.502, 0.010, 0.005, 0.483, 0.000],
             ...
             [0.433, 0.200, 0.000, 0.367, 0.000], [0.000, 0.000, 0.500, 0.000,
                                                   0.500]]
...
def greedy_reward(used_gama, computing_time, energy_consumption, finish_time, start_time, qos, p_idle):
    request_num = math.ceil((finish_time - start_time) / qos)
    e_self_it = (energy_consumption + p_idle * (qos - computing_time)) * request_num
    e_self_cooling = used_gama * e_self_it
...
def get_reward(used_gama, activated_server_racks, processing_request, request_id, computing_time, energy_consumption,
               finish_time, start_time, qos, p_idle):
    request_num = math.ceil((finish_time - start_time) / qos)
    e_self_it = (energy_consumption + p_idle * (qos - computing_time)) * request_num
    e_self_cooling = used_gama * e_self_it
    rack_id = processing_request[request_id][1]
    ...
    e_other_cooling = used_gama * e_other_it
    q = math.log(1 + math.exp(100 * (computing_time - qos) / qos))
...
for i in range(episode):
    ...
    for indexes in df.index:
        ...
        if flag == 0:  # 某一个请求结束,则更改STATE，即hardwareNumber
            ...
            use_gama = gama[action]
            reward = get_reward(use_gama, activated_server_racks, processing_request, request_id, c_time,
                                e_consumption, outTime, inTime, QoS, idle_power[action])
