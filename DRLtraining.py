        ...
        for action in range(4):
            for ii in range(M[action]):
                if server_rack[action][ii][0]==1:
                    ...
                    energy = [CPUEnergyW[DNNType], CPUEnergyC[DNNType], GPUEnergyW[DNNType], GPUEnergyC[DNNType]][action]
                    e_active+=energy
    ...
    for indexes in df.index:
       # print(indexes)
        ...
        if indexes<len(df.index)-1:
            ...
            f.write('time slice: '+str(timeline)+' active energy:'+str(e_active)+' idle energy: '+str(e_idle)+'\n')
